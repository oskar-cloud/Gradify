window.LibraryView = (() => {
  ...
  async function show(container) {
    container.innerHTML = `
      <div class="view-header">
        <h1 class="view-title">Liked Songs</h1>
        <p class="view-subtitle">Your saved tracks</p>
      </div>
    ...
    try {
      ...
      if (tracks.length === 0) {
        content.innerHTML = `<div class="empty-state">${Icons.heart}<p>No liked songs yet</p></div>`;
        return;
      }
      ...
      content.querySelectorAll('.track-row').forEach(row => {
        row.addEventListener('click', async () => {
          try {
            await window.gradify.spotify.play({ uris: [row.dataset.uri] });
  ...
  function trackRow(t, num) {
    ...
    return `
      <div class="track-row ${playing?'playing':''}" data-uri="${t.uri}">
